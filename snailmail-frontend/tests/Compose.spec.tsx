...
test.beforeEach(async ({ page }) => {
  await page.goto('http://localhost:5173'); //Visit the main page
...
test('user can send email via compose screen', async ({ page }) => {
  ...
  const response = await page.waitForResponse('**/mail');  // This waits for a network response matching the /mail URL
...
//TODO: test for backend being down, HTTP request failing
...
test('closes the compose component when X is clicked', async ({ page }) => {
    ...
    //Assert that Compose is no longer in the DOM
